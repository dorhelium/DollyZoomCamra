...
public class DOFCamera {
	...
	DoubleParameter focalLength = new DoubleParameter( "focal length mm", 35, 18, 500 );     // (35, 18, 500)
	...
	DoubleParameter filterRate = new DoubleParameter( "filter rate", 0.5, 0, 1); //0.5, 0, 1
    ...
    public void updateFilteredQuantities() {
		...
		if ( dollyZoom.getValue() ) {
			...
			focalLength.setValue(dolly.getValue());
			
			
	    	double focusPlaneHeight = Math.tan( Math.toRadians( fovy.getValue()/2 ) ) * focalLength.getValue() * 2;
	    	
	    	fovy.setValue(Math.toDegrees((Math.atan(focusPlaneHeight/2/dolly.getValue()))));
    ...
    JPanel getControls() {
    	...
    	focalLength.addParameterListener( new ParameterListener<Double>() {
    		...
    		public void parameterChanged(Parameter<Double> parameter) {
    			...
    			// change this!
    			double value = 2 * Math.toDegrees(Math.atan(sensorHeight.getValue()/2/focalLength.getValue())); 
    	...
    	fovy.addParameterListener( new ParameterListener<Double>() {
    		...
    		public void parameterChanged(Parameter<Double> parameter) {
    			...
    			double value = sensorHeight.getValue()/2/Math.tan(Math.toRadians(fovy.getValue()/2)); 
    ...
    public void setupProjection( GLAutoDrawable drawable, int i ) {
    	...
    	double focalPlaneHeight = sensorHeight.getValue()/focalLength.getValue() * focusDesired.getValue();
    	double focalPlaneWidth = drawable.getSurfaceWidth()/drawable.getSurfaceHeight()*focalPlaneHeight;
    	double s = getEffectivePupilRadius();
    	double xOffset = s * p.x ;
    	double yOffset = s * p.y ;
    	
    	//left[0], right[1], top[2], bottom[3], z[4]
    	double[] focalPlaneLocation = {-focalPlaneWidth/2, focalPlaneWidth/2, focalPlaneHeight/2, -focalPlaneHeight/2, focusDesired.getValue()};
    	//System.out.println(focalPlaneLocation[4]);
    	double nearRight = ((focalPlaneLocation[1] - xOffset) / focalPlaneLocation[4])* near.getValue();
    	double nearLeft = ((focalPlaneLocation[0] - xOffset) / focalPlaneLocation[4])* near.getValue() ;
    	double nearTop = ((focalPlaneLocation[2] - yOffset) / focalPlaneLocation[4])* near.getValue() ;
    	double nearBottom = ((focalPlaneLocation[3] - yOffset) / focalPlaneLocation[4])* near.getValue() ;
    	
    	
		
		gl.glFrustum(nearLeft, nearRight, nearBottom, nearTop, near.getValue(), far.getValue() );
    ...
    public void setupViewingTransformation( GLAutoDrawable drawable, int i ) {
		...
		glu.gluLookAt( eye.x + xOffset , eye.y + yOffset , eye.z, lookAt.x+ xOffset , lookAt.y + yOffset, lookAt.z, 0, 1, 0);
    ...
    public void drawFocusPlane( GLAutoDrawable drawable ) {
    	...
    	double ratio = drawable.getSurfaceWidth()/drawable.getSurfaceHeight();
    	double focusPlaneHeight = sensorHeight.getValue()/2/focalLength.getValue() * focusDistance  * 2;
    	double focusPlaneWidth = focusPlaneHeight * ratio;
    	...
    	//top right
    	gl.glVertex3d(focusPlaneWidth /2, focusPlaneHeight/2, -focusDistance);
    	//top left
    	gl.glVertex3d(- focusPlaneWidth /2, focusPlaneHeight/2, -focusDistance);
    	//bottom left
    	gl.glVertex3d(- focusPlaneWidth /2, - focusPlaneHeight/2, -focusDistance);
    ...
    public void drawSensorPlane( GLAutoDrawable drawable ) {
    	...
    	double convertedSensorHeight = sensorHeight.getValue()/1000;
    	double convertedFocalLength = focalLength.getValue()/1000;
    	double ratio = drawable.getSurfaceWidth()/drawable.getSurfaceHeight();
      	double sensorWeidth = convertedSensorHeight * ratio;
    	...
    	//top right
    	gl.glVertex3d(sensorWeidth /2, convertedSensorHeight/2, -convertedFocalLength);
    	//top left
    	gl.glVertex3d(- sensorWeidth /2, convertedSensorHeight/2, -convertedFocalLength);
    	//bottom left
    	gl.glVertex3d(- sensorWeidth /2, - convertedSensorHeight/2, -convertedFocalLength);
